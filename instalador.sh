...
help(){
    ...
    echo "  -h, --help           VER AYUDA"
    echo "  -k, --keep           MANTENER LA config.json PARA ACTUALIZAR"
    echo "      --remove         ELIMINAR v2ray,xray && multi-v2ray"
    echo "                       SIN PARAMETROS PARA NUEVA INSTALACION"
    return 0
}
...
removeV2Ray() {
    ...
    colorEcho ${GREEN} "DESINSTALACION EXITOSA!"
}
...
checkSys() {
    #检查是否为Root
    [ $(id -u) != "0" ] && { colorEcho ${RED} "Error: DEBE SER ROOT PARA EJECUTAR EL SCRIPT"; exit 1; }
    ...
    if [[ `command -v apt-get` ]];then
        ...
        colorEcho $RED "SISTEMA OPERATIVO NO SOPORTADO"
        exit 1
    fi
}
...
timeSync() {
    if [[ ${INSTALL_WAY} == 0 ]];then
        echo -e "${Info} Sincronizando Hora.. ${Font}"
...
installFinish() {
    ...
    colorEcho  ${GREEN} "multi-v2ray ${WAY} instalado!\n"
    ...
    if [[ ${INSTALL_WAY} == 0 ]]; then
        ...
        echo -e "por favor ingrese 'v2ray' para administrar v2ray\n"
    fi
}
...
while true; do
    read -p "¿DESEAS INSTALAR BADVPN (habilitar para juegos-videollamadas)?  ESCRIBA YES o NO......"   yn
    case $yn in
