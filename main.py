...
def service_manage():
    show_text = (_("INICIAR {}".format(run_type)), _("DETENER {}".format(run_type)), _("REINICIAR {}".format(run_type)), _("{} ESTADO".format(run_type)), _("{} log".format(run_type)))
    print("")
...
def profile_alter():
    show_text = (_("CAMBIAR EMAIL"), _("CAMBIAR UUID"), _("CAMBIAR alterID"), _("CAMBIAR PUERTO"), _("CAMBIAR PROTOCOLO"), _("CAMBIAR OPC. TLS"), 
                _("CAMBIAR OPC tcpFastOpen"), _("CAMBIAR PUERTO DYN"), _("CAMBIAR shadowsocks"), _("CAMBIAR CLAVE shadowsocks"), _("CDN METODO (SOLO CON DOMINIO)"))
    print("")
    for index, text in enumerate(show_text): 
        print("{}.{}".format(index + 1, text))
    choice = loop_input_choice_number(_("POR FAVOR SELECCIONE: "), len(show_text))
...
def global_setting():
    show_text = (_("{} ESTADISTICAS DE TRAFICO".format(run_type.capitalize())), _("Iptables Traffic Statistics"), _("Ban Bittorrent"), _("Schedule Update {}".format(run_type.capitalize())), _("Clean {} Log".format(run_type.capitalize())), _("Change Language"))
    print("")
    for index, text in enumerate(show_text): 
        print("{}.{}".format(index + 1, text))
    choice = loop_input_choice_number(_("POR FAVOR SELECCIONE: "), len(show_text))
...
def menu():
    ...
    while True:
        ...
        print("")
        show_text = (_("1.ADM. SISTEMA".format(run_type.capitalize())), _("2.ADM. CLIENTES"), _("3.ADM. CONFIG."), _("4.VER INFO CLIENTES"), _("5.AJUSTES GLOBALES"), _("6.ACTUALIZAR {}".format(run_type.capitalize())), _("7.GENERAR JSON"))
        for index, text in enumerate(show_text): 
            if index % 2 == 0:
