...
class CommonSelector:
    ...
    def select(self):
        ...
        if not choice.isnumeric():
            raise RuntimeError(_('ERROR, REVISE EL NUMERO INGRESADO'))
...
class ClientSelector(Selector):
    def __init__(self, action):
        ...
        if _("del") in action and self.list_size == 1:
            print(ColorStr.red(_("EL ULTIMO NODO NO PUEDE SER ELIMINADO!!!")))
            self.group = None
    ...
    def select_client(self):
        ...
        if self.list_size < 10:
            choice = readchar("{} {}: ".format(_("POR FAVOR INGRESE EL NUMERO"), self.action))
        else:
            choice = input("{} {}: ".format(_("POR FAVOR INGRESE EL NUMERO"), self.action))

        if not choice:
            return
        if not choice.isnumeric():
            print(ColorStr.red(_('ERROR, REVISE EL NUMERO INGRESADO')))
            return

        choice = int(choice)
        if choice < 1 or choice > self.list_size:
            print(ColorStr.red(_('LO INGRESADO ESTA FUERA DE RANGO!')))
        else:
            find = False
...
class GroupSelector(Selector):
    def __init__(self, action):
        ...
        if _("del") in action and len(self.group_list) == 1:
            print(ColorStr.red(_("EL ULTIMO GRUPO NO PUEDE SER ELIMINADO!!!")))
            self.group = None
    ...
    def select_group(self):
        ...
        if len(self.group_list[-1].tag) == 1:
            choice = readchar("{} {}: ".format(_("POR FAVOR INGRESE EL GRUPO"), self.action))
        else:
            choice = input("{} {}: ".format(_("POR FAVOR INGRESE EL GRUPO"), self.action))
        group_list = [x for x in self.group_list if x.tag == str.upper(choice)]
        if len(group_list) == 0:
            print(ColorStr.red('{0} {1} {2}'.format(_("ERROR, REVISE EL GRUPO INGRESADO"), choice, _("exist"))))
            self.group = None
        else:
