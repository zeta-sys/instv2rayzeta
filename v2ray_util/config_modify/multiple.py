...
def new_port(new_stream=None):
    ...
    while True:
        ...
        new_port = input("{0} {1}, {2}: ".format(_("GENERANDO PUERTO ALEATORIO"), ColorStr.green(str(new_random_port)), _("PRESIONE ENTER PARA USAR O INGRESE UN PUERTO")))
        ...
        if not new_port:
            ...
            if not new_port.isnumeric():
                ...
                print(_("PUERTO EN USO, SELECCIONE OTRO!"))
                continue
        break
        ...
        if type(group.node_list[0]) in (Vmess, Vless, Trojan): 
            while True:
                ...
                if not is_email(email):
                    print(_("NO ES UN EMAIL, INGRESE UNO"))
                    continue
                ...
                for loop_group in group_list:
                    for node in loop_group.node_list:
                        if node.user_info == None or node.user_info == '':
                            ...
                            print(_("EMAIL YA REGISTRADO, INGRESE OTRO"))
...
def del_port():
    ...
    if group == None:
        ...
        print(_("PUERTO A ELIMINAR "))
        print(group)
        choice = readchar(_("ESTA SEGURO?(y/n): ")).lower()
        if choice == 'y':
            nw = NodeWriter()
            nw.del_port(group)
