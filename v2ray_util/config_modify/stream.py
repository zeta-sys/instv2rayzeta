...
class StreamModifier:
    ...
    def select(self, sType):
        ...
        if sType in (StreamType.TCP_HOST, StreamType.WS):
            host = input(_("INGRESE DOMINIO FALSO: "))
            kw['host'] = host
...
def modify(group=None, sType=None):
    ...
    if group == None:
        ...
        if sType != None:
            ...
            print(_("PROTOCOLO CAMBIADO CON EXITO!!!"))
            return

        if need_restart:
            print("")
            print("{}: {}".format(_("PROTOCOLOS"), group.node_list[0].stream()))
        ...
        print("")
        choice = input(_("SELECCIONE NUEVO PROTOCOLO: "))

        if not choice.isdecimal():
            print(_("INGRESE UN NUMERO!!!"))
        else:
            choice = int(choice)
